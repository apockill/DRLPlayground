...
dataset = Dataset("./images/", .2, load_resolution=(32, 32))
img_rows, img_cols, img_chns = 32, 32, 3
img_rows, img_cols, img_chns = dataset.height, dataset.width, dataset.channels
...
decoder_upsample = Dense(filters * int(img_rows / 2 * img_cols / 2), activation='relu')
...
x_decoded_mean_squash = decoder_mean_squash(x_decoded_relu)
"""
# save all 3 old for future use - especially generator
vae.save('./old/VAE_ld_%d_conv_%d_id_%d_e_%d_vae.h5' % (latent_dim, num_conv, intermediate_dim, epochs))
encoder.save('./old/Encoder_ld_%d_conv_%d_id_%d_e_%d.h5' % (latent_dim, num_conv, intermediate_dim, epochs))
generator.save('./old/Decoder_ld_%d_conv_%d_id_%d_e_%d.h5' % (latent_dim, num_conv, intermediate_dim, epochs))

# save training history
fname = './old/cifar10_ld_%d_conv_%d_id_%d_e_%d_history.pkl' % (latent_dim, num_conv, intermediate_dim, epochs)
